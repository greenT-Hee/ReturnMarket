...
//--- medium ---
export const M_btn = ({children, btnFn, type="button"}) => {
  return (
    <MediumBtnStyle type={type} onClick={btnFn}>{children}</MediumBtnStyle>
  )
}
...
export const MS_btn_disable = ({children, btnFn}) => {
  return (
    <MSBtnStyle $disabled='true' disabled type="button" >{children}</MSBtnStyle>
  )
}
